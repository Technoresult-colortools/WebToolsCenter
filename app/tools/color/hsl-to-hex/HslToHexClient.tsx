...
export default function HslToHex() {
  ...
  const handleConvert = () => {
    ...
    if (isNaN(h) || h < 0 || h >= 360) {
      ...
      toast.error('Invalid Hue value. Please enter a number between 0 and 359.');
      return;
    }

    if (isNaN(s) || s < 0 || s > 100) {
      setError('Please enter a valid Saturation value (0-100).');
      toast.error('Invalid Saturation value. Please enter a number between 0 and 100.');
      return;
    }

    if (isNaN(l) || l < 0 || l > 100) {
      setError('Please enter a valid Lightness value (0-100).');
      toast.error('Invalid Lightness value. Please enter a number between 0 and 100.');
      return;
    }
    ...
    setHexValue(hex);
    toast.success(`Conversion successful! Hex value: ${hex}`);
  };

  return (
    <ToolLayout
      title="HSL to Hex Converter"
      description="Convert HSL Color Codes to Hex"
    >
      <Toaster position="top-right" /> {/* Toaster to display toast notifications */}
