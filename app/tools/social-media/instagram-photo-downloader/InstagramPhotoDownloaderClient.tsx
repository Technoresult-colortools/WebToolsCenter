...
export default function InstagramPhotoDownloader() {
    ...
    const handleSearch = async () => {
        ...
        if (!validateUrl(url)) {
            toast.error('Invalid Instagram URL. Please enter a valid post or reel URL.');
            setIsLoading(false);
            return;
        }
    ...
    const handleProfileSearch = async () => {
        ...
        if (!validateUrl(url)) {
            toast.error('Invalid Instagram URL. Please enter a valid profile URL.');
            setIsLoading(false);
            return;
        }
    ...
    const handleProfileDownload = () => {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `instagram_profile_${profileData.username}_${Date.now()}.json`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
